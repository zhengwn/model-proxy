...
#[tauri::command]
pub async fn save_config(state: State<'_, AppState>, mut config: Config) -> Result<(), String> {
    let _lock = state.config_lock.lock().await;
...
#[tauri::command]
pub async fn switch_provider(app_state: State<'_, AppState>, name: String) -> Result<(), String> {
    let _lock = app_state.config_lock.lock().await;
...
#[tauri::command]
pub async fn get_providers(app_state: State<'_, AppState>) -> Result<ProvidersInfo, String> {
    let _lock = app_state.config_lock.lock().await;
...
#[tauri::command]
pub async fn get_model_routes(app_state: State<'_, AppState>) -> Result<Vec<ModelRoute>, String> {
    let _lock = app_state.config_lock.lock().await;
    let mut config = get_config_internal(&app_state.config_path)?;
    config.normalize();
...
// ---- Kiro Admin API proxy commands ----
//
// These commands make HTTP requests to the proxy's admin API endpoints.
...
#[tauri::command]
pub async fn kiro_list_credentials(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn kiro_get_endpoint_health(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    admin_get(&state, "endpoints/health").await
}

/// Get thinking config.
#[tauri::command]
pub async fn kiro_get_thinking(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    admin_get(&state, "thinking").await
}

/// Set thinking config.
#[tauri::command]
pub async fn kiro_set_thinking(state: State<'_, AppState>, mode: String) -> Result<serde_json::Value, String> {
    admin_post(&state, "thinking", serde_json::json!({ "mode": mode })).await
}

/// Get proxy settings.
#[tauri::command]
pub async fn kiro_get_settings(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn kiro_get_lb_config(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    admin_get(&state, "config").await
}

/// Set load balancing mode.
#[tauri::command]
pub async fn kiro_set_lb_config(state: State<'_, AppState>, mode: String) -> Result<serde_json::Value, String> {
